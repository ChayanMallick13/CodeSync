...
const ShowMembers = ({ permittedUsers, activeUsers, owner,thisUsersPermissions,
  socket,showChat
 }) => {
  ...
  async function handlePermissionChange(body,targetUser,targetId,newPos,oldPos){
    console.log(body);
    const res = await changePermissions(body,setDisableBtn,setShowChangeOptions);
    if(res.success){
  ...
  return (
    ...
    `}>
      ...
      {permittedUsers.map((userParams, key) => {
        return (
          <div key={key} className="flex gap-x-2 items-center relative" ref={boxRef}>
            <img
              referrerPolicy="no-referrer"
              alt="user"
            ...
            <div>
              ...
              ${userParams.permissions.delete?('text-indigo-600'):(userParams.permissions.write?
              ('text-orange-600'):('text-blue-600'))}
              flex items-center justify-between
              `}>
                {userParams.permissions.delete?('Moderator'):(userParams.permissions.write?('Writer'):('Reader'))}
               {
                thisUsersPermissions.delete&&<button onClick={()=>{setShowChangeOptions(prev => !prev)}}>
                  <SlOptionsVertical/>
                </button>
               }
              </p>
              <div className="flex items-center justify-between">
                ...
                >
                  {activeusersId.includes(userParams?._id) ? "Active" : "Inactive"}
                </p>
                <div
                  className="h-[10px] w-[10px] rounded-full"
                  style={{
            ...
            {
              showChangeOption&&<div className="absolute -bottom-10 bg-slate-700 border-slate-100/60 border-[1px] p-2 w-full
              rounded-xl flex flex-col gap-y-2
              "
              >
                <h3 className="font-bold">Change Role for {userParams.firstName} {userParams.lastName}</h3>
                <select defaultValue={getDefaultUserRole(userParams.permissions)}
                ...
                <div className="flex items-center justify-around">
                  <button onClick={()=>{setShowChangeOptions(false)}}
                  ...
                  onClick={(e)=>{
                    ...
                    handlePermissionChange(body,`${userParams.firstName} ${userParams.lastName}`
                    ,userParams._id,userPosition,oldPos);
                  }}
                  disabled={disableBtn}
                  >
                    Save
